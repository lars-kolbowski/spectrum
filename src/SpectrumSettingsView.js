	...
	events: function() {
		...
		checkInputsForValidity: function(formData){
			...
			var invalidChars = function(input, unknownCharPattern){
				...
				if (match){
					console.log('Invalid character found!:' + match[0]);
					console.log(match);
					return match;
				}
				return false;
			...
			if (invalidChars(formData['peps'].value, /([^GALMFWKQESPVICYHRNDTa-z;#0-9(.)\-]+)/)){
				console.log('Invalid char in peptide sequence!')
				return false;
			}

			//peakList
			if (invalidChars(formData['peaklist'].value, /([^0-9\.\s]+)/)){
				console.log('Invalid char in peaklist!')
				return false;
			}
			//clModMass
			if (invalidChars(formData['clModMass'].value, /([^0-9\.]+)/)){
				console.log('Invalid char in crossLinkerModMass!')
				return false;
			}
			//precursor charge state
			if (invalidChars(formData['preCharge'].value, /([^0-9]+)/)){
				console.log('Invalid char in charge state!')
				return false;
			}
			//ms2Tolerance
			if (invalidChars(formData['ms2Tol'].value, /([^0-9\.]+)/)){
				console.log('Invalid char in ms2Tolerance!')
				return false;
			}
